...
const Notification = ({ title, message, type, id }) => {
  ...
  return (
    <div
      ...
      <span className='absolute top-0 bottom-0 right-0 px-2 py-3' onClick={hideNotification}>
        <svg className={`fill-current h-6 w-6 ${type === 'error' ? 'text-red-500' : 'text-green-500'}`} role='button' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 20 20'>
          <title>Close</title>
