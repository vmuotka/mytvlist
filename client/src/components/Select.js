    ...
    <div className={`px-3 mb-6 md:mb-0  ${props.className}`}>
      ...
      <div className='relative'>
        <select
          ...
          id='label'
        >
          {props.options.map(option => <option className='appearance-none bg-gray-300 focus:bg-gray-500 capitalize' value={option.value ? option.value : option} key={option.name ? option.name : option}>{option.name ? option.name : option}</option>)}
        </select>
